...
const DUE_DATE_DAYS = Array.from({ length: 31 }, (_, i) => i + 1); // 1st to 31st
...
export default function AddCardScreen() {
  ...
  useEffect(() => {
    fetchCardCatalog().then(setCatalog).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        <Text style={styles.label}>Card (pick or type custom)</Text>
        <TextInput
          style={styles.input}
        ...
        {loading ? (
          ...
          <View style={styles.catalogList}>
            {filteredCatalog.slice(0, 8).map((card) => (
              <TouchableOpacity
                key={card.id}
        ...
        <TouchableOpacity style={styles.customBtn} onPress={useCustom}>
          <Text style={styles.customBtnText}>Use custom name</Text>
        </TouchableOpacity>
